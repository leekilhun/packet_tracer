...
int util::constrain(int amt, int low, int high)
{
  return ((amt) < (low) ? (low) : ((amt) > (high) ? (high) : (amt)));
}

bool util::between(int amt, int low, int high)
{
  bool ret = true;
  if (amt < low) {
    ret = false;
...
int util::DwToInt(uint8_t* bytes)
{
  int a = bytes[0] & 0xFF;
  a |= ((bytes[1] << 8) & 0xFF00);
...
uint32_t util::DwToUint(uint8_t* bytes)
{
...
{
  count = util::constrain(count, 0, (int)strlen(pdata));
  int i = 0;
  for (; i < count; i++) {
    pdest[i] = pdata[i];
  }
...
bool util::IsEqualStr(char* str_rf, char* str_cp, bool is_ignore)
{
  bool ret = true;
    ...
    case state_t::value:
    {
      ...
      case 'f': case 'F':
      {
        ...
        if (begin && end)
        {
            ...
            bool is_true = util::IsEqualStr(tmp_bool, "true", true);
      ...
      while (t_curr < t_arg_size)
      {
        ...
        // fine key and value
        {
          ...
          case 'f': case 'F':
          {
            memcpy(m_vData[m_incNo].key, key_str.data(), key_str.size());
            m_vData[m_incNo].is_array = true;
            bool is_true = util::IsEqualStr(tmp_p[i], "true", true);
            m_vData[m_incNo].yesorno = is_true;
            m_vData[m_incNo].type = valueType_e::boolian;
            if ((++m_incNo < m_dataMax) == false)
            {
