...
class moons_motor : public IAxis
{
	/****************************************************
	 *	data
	 ****************************************************/
...
private:
   ...
   ****************************************************/
public:
	moons_motor(common::Attribute_t& common_data, cfg_t& cfg);
	virtual ~moons_motor();
...
private:
#ifdef _DEBUG
  static void cliMoonsTest(void* obj, cli_args_t* args);
#endif
  ...
  bool sendPacketWaitRxResp(uint8_t* p_data, uint32_t length, uint32_t timeout = 100);

public:
  /*  함수*/
  void AttCallbackFunc(void* obj, evt_cb cb);

  errno_t GetReg(moons_SS_reg_e reg);
  int GetPosition();
  errno_t  GetMotorData();
 ...
 // void LoadingParamterData();
  ...
  int SendPacket(char** packets, int cnt);
};
