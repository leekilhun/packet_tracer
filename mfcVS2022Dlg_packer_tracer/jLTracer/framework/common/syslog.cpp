...
/* 전역 함수로 구현 */
buffer::_que<logdat_t> log_table[static_cast<int>(loglevel::_max)]{ SYS_LOG_BUFF_LIST_MAX, };
...
uint32_t syslog_Write(loglevel level, const int obj, const char* file_info, const char* file, const char* func, const int line, const char* fmt, ...)
{
  uint32_t ret = 0;
  if (is_init != true) return ret;
...
sysLog::sysLog() :m_isInit(false), m_size(0)
{
  m_isInit = true;
}
...
uint32_t sysLog::syslog_Write(loglevel level, const int obj, const char* file_info, const char* file, const char* func, const int line, const char* fmt, ...)
{
  uint32_t ret = 0;
  if (is_init != true) return ret;
