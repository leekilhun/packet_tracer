 ...
 * 
 * 수정
 * '22.10.04 날짜 표시 수정 [2022-10-04 13:38:41'596.proj_AutoSplicing] -> [2022-10-04 13:45:51'11] 
 */
...
#define LOG_PRINT(fmt, ...)		syslog_Write(loglevel::info, 0,SYSLOGFILE_INFO, (__FILE__), __LOGFUNC__, __LINE__,  fmt, ##__VA_ARGS__)
#define ERR_PRINT(fmt, ...)		syslog_Write(loglevel::err, 0,SYSLOGFILE_ERR, (__FILE__), __LOGFUNC__, __LINE__,  fmt, ##__VA_ARGS__)
#define LTRACE(fmt, ...)   syslog_Write(loglevel::info, 0, SYSLOGFILE_INFO, (__FILE__), __LOGFUNC__, __LINE__,  fmt, ##__VA_ARGS__);\
...
logdat_t syslog_Get (int level = static_cast<int>(loglevel::info));
logdat_t syslog_Pop(uint32_t addr, int level = static_cast<int>(loglevel::info));
uint32_t syslog_Available(int level = static_cast<int>(loglevel::info));
